...
def read_annotation_file(label, action_verb_mapping, object_mapping, tool_mapping, label_mapping):
    ...
    assistant_text_general = ("I will describe the primitive task in a structured way, using four elements: an action verb, a manipulated object, a target object, and a tool. \n")
    ...
    assistant_text_lh_compositional = "Based on the observation of the worker's left hand's motion and the involved objects, analyze each action element: \n"
    if label == "null":
        description = f"The left hand of the worker did nothing related to the assembly task. Therefore the all the action elements are Null \n "
    else:
        description =  f"The action verb is {action_verb}; manipulated object is {manipulated_object}; target object is {target_object}; the tool is {tool}. \n"
    assistant_text_lh_compositional = assistant_text_lh_compositional + description
    
    assistant_text_lh_full = "Based on the above analysis, analyze the primitive task performed by the left hand of the worker: \n"    
    if label == "null":
        description_full = f"The left hand of the worker did nothing related to the assembly task. Therefore the primitive task is Null \n "
    else:
        description_full =  f"The left hand of the worker {semantics}\". "
    assistant_text_lh_full = assistant_text_lh_full + description_full

    assistant_text = assistant_text_general + assistant_text_lh_compositional  + assistant_text_lh_full 
            ...
            # or however your naming convention is set
                data_entry = {
                    ...
                    "videos": [
                        ...
                        "havid_sub_videos_crop/lh_v0/train/" + os.path.splitext(filename)[0] + ".mp4"
                    ]
                }
...
def main():
    ...
    split_videos_folder = "./havid_sub_videos_crop/lh_v0/train"
    ...
    output_json = "./json/CCFT_havid_sub_videos_crop_lh_v0_simple_v3.json"
