...
def read_annotation_file(label, action_verb_mapping, object_mapping, tool_mapping, label_mapping):
    ...
    if label == "null":
        assistant_text_lh["verb"] = "Observing the motion of the worker's left hand, the left hand of the worker did nothing related to the assembly task. Therefore the action verb is null.\n"
        assistant_text_lh["manipulated_object"] = "Focusing on the object that the worker's left hand is interacting with, the manipulated object is null.\n"
        assistant_text_lh["target_object"] = "Looking at the object that the manipulated object is assembled onto, the target object is null.\n"
        assistant_text_lh["tool"] = "Observing the tool that the worker is using, the tool is null."
        assistant_text_lh["complete"] = assistant_text_lh_cap + "Observing the motion of the worker's left hand, the left hand of the worker did nothing related to the assembly task. Therefore, the action verb is null. The manipulated object is null. The target object is null. The tool is null. Based on the above analysis, the primitive task is null.\n "   
    else:
        assistant_text_lh["verb"] = f"Observing the motion of the worker's left hand, the assembly action verb is {action_verb}.\n"
        assistant_text_lh["manipulated_object"] = f"Focusing on the object that the worker's left hand is interacting with, the manipulated object is {manipulated_object}.\n"
        assistant_text_lh["target_object"] = f"Looking at the object that the manipulated object is assembled onto, the target object is {target_object}.\n"
        assistant_text_lh["tool"] = f"Observing the tool that the worker is using, the tool is {tool}.\n"
        assistant_text_lh["complete"] = assistant_text_lh_cap + f"Observing the motion of the worker's left hand, the action verb is {action_verb}. Focusing on the object that the worker's left hand is interacting with, the manipulated object is {manipulated_object}. Looking at the object that the manipulated object is assembled onto, the target object is {target_object}. Observing the tool that the worker is using, the tool is {tool}. Based on the above analysis, the primitive task is {semantics}.\n"
    ...
    for filename in os.listdir(split_videos_folder):
        if filename.lower().endswith(".mp4"):
            ...
            # or however your naming convention is set
                ...
                #         "havid_sub_videos_crop_balanced/lh_v0/train/" + os.path.splitext(filename)[0] + ".mp4"
                #     ]
                # }
            
                # json_data.append(data_entry)
                
                # Assuming user_text and assistant_text have the same keys (e.g., "verb", "manipulated_object", etc.)
                for key in user_text:
                        # Create a new entry for each key
                    data_entry = {
                        "messages": [
                            {
                                "content": user_text[key],
                                "role": "user"
                            },
                            {
                                "content": assistant_text[key],
                                "role": "assistant"
                            }
                        ],
                        "videos": [
                        # Use the same video path for all entries
                            f"havid_sub_videos_crop_balanced/lh_v0/train/{os.path.splitext(filename)[0]}.mp4"
                            ]
                        }
                    json_data.append(data_entry)
...
def main():
    ...
    split_videos_folder = "./havid_sub_videos_crop_balanced/lh_v0/train"
    ...
    output_json = "./json/CCFT_havid_sub_videos_crop_balanced_lh_v0_multi_QA_v2.json"
