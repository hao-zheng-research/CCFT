...
def read_annotation_file(label, action_verb_mapping, object_mapping, tool_mapping, label_mapping):
    ...
    # user_text = user_text + """Analyze the assembly primitive task being performed by the worker's left hand through systematic observation:  
    ...
    user_text += "Based on the observation, describe the primitive task in a structured way, using four elements: an action verb, a manipulated object, a target object, and a tool."
    valid_classes = """Possible classes for each action element and primitive task :\n- Action verbs: ["insert", "slide", "place", "rotate", "screw", "wrong", "none"]
    ...
    if label == "null":
        CoT_main = f"""
                   The worker's left hand is not assembling anything onto anything, suggesting the action verb \"None\". 
                   Therefore, the manipulated object, target object and tool are all \"None\".\n
                   """
        CoT_template = "chain_of_thought: " + CoT_head + CoT_main
    else:
...
def gather_split_frames_annotations(split_frames_folder, action_verb_mapping, object_mapping, tool_mapping, label_mapping):
    ...
    for group_key, frames in grouped_images.items():
        ...
        for idx, (frame_idx, filename) in enumerate(sorted_frames):
            placeholder = f"<image>"
            image_dict.append(f"havid_sub_frames_crop/lh_v0/train/{filename}")
        ...
        image_tags = "\n".join(image_paths)
        user_text = user_text.replace("<image>", f"{image_tags}")
...
def main():
    ...
    split_frames_folder = "./havid_sub_frames_crop/lh_v0/train"
    ...
    output_json = "./json/CCFT_havid_sub_frames_crop_lh_v0_cot.json"
