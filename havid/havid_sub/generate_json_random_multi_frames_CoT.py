...
def parse_splitted_filename(filename):
    ...
    *base_parts, label, idx_str, idx_extract, idx_frame = parts
    ...
    return (base_name, label, clip_index, idx_extract, frame_index)
    ...
    for filename in os.listdir(split_frames_folder):
        if filename.lower().endswith(".jpg"):
            ...
            if parsed is not None:
                base_name, label, clip_index, idx_extract, frame_index = parsed
                group_key = (base_name, label, clip_index, idx_extract)
                grouped_images[group_key].append((frame_index, filename))
                
    for group_key, frames in grouped_images.items():
        base_name, label, clip_index, idx_extract = group_key
        ...
        for idx, (frame_idx, filename) in enumerate(sorted_frames):
            placeholder = f"<image>"
            image_dict.append(f"havid_sub_frames_crop_extend/rh_v0/train/{filename}")
...
def main():
    ...
    split_frames_folder = "./havid_sub_frames_crop_extend/rh_v0/train"
    ...
    output_json = "./json/CCFT_havid_sub_frames_crop_extend_rh_v0_cot.json"
