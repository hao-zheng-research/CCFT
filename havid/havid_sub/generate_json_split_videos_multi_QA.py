...
def read_annotation_file(label, action_verb_mapping, object_mapping, tool_mapping, label_mapping):
    ...
    if label == "null":
        ...
        assistant_text_lh["manipulated_object"] = "The worker's left hand is not assembling anything, so the manipulated object is null.\n "
        assistant_text_lh["target_object"] = "The worker's left hand is not assembling anything, so the target object is null.\n "
        assistant_text_lh["tool"] = "The worker is not using any tool, so the tool is null."
        assistant_text_lh["complete"] = assistant_text_lh_cap + "By analysing the four action elements, the primitive task is null.\n "   
    else:
        assistant_text_lh["verb"] = f"Observing the motion of the worker's left hand, the assembly action verb is {action_verb}. "
        assistant_text_lh["manipulated_object"] = f"Observing the object that the worker's left hand is interacting with, the manipulated object is {manipulated_object}. "
        ...
        assistant_text_lh["complete"] = assistant_text_lh_cap + f"By analysing the four action elements, the primitive task is {semantics}. \n"
...
def main():
    ...
    output_json = "./json/CCFT_havid_sub_videos_crop_balanced_lh_v0_multi_QA.json"
